...
export function DataTable<TData, TValue>({
  ...
  const executeDeleteSelected = async () => {
    ...
    try {
      ...
      await Promise.all(
        idsToDelete.map((id) => api.delete(`/api/leads/${id}`)),
      );
  ...
  const handleAddLead = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post("/api/leads", {
        ...
        status: (status === "Lead" ? "Open" : status) as
          | "Open"
          | "Active"
          | "Closed"
          | "Lost",
      ...
      const mappedLead = {
        ...
        priority: createdLead.priority
          ? createdLead.priority.toLowerCase()
          : "medium",
    ...
    try {
      ...
      const mappedLead = {
        ...
        priority: updatedLead.priority
          ? updatedLead.priority.toLowerCase()
          : "medium",
      ...
      setLocalData((prev) =>
        prev.map((lead: any) =>
          lead.id === editingLeadId ? mappedLead : lead,
        ),
      );
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        if (parsedRows.length === 0) {
          toast.error(
            "The uploaded CSV file appears to be empty or has an invalid structure.",
          );
          setIsImporting(false);
          return;
        }
        ...
        for (const row of parsedRows) {
          ...
          const mappedLead = {
            ...
            lastSeen: createdLead.lastContactDate || new Date().toISOString(),
            "next date":
              createdLead.nextFollowUpDate || new Date().toISOString(),
            image:
              "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=60",
          };
        ...
        if (successLeads.length > 0) {
          ...
          toast.success(
            `Successfully imported ${successLeads.length} leads.${skippedCount > 0 ? ` Skipped ${skippedCount} invalid rows.` : ""}`,
          );
        } else {
          toast.warning(
            `No leads were imported. Skipped ${skippedCount} invalid rows.`,
          );
        }
      } catch (err) {
  ...
  const allUniqueTags = Array.from(
    new Set(localDataArray.flatMap((lead: any) => lead.tags || [])),
  ).filter(Boolean);
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mt-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <DropdownMenu>
              <div className="flex flex-col gap-1">
                <DropdownMenuLabel className="text-[10px] font-bold text-muted-foreground/80 uppercase p-0">
                  Status
                </DropdownMenuLabel>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-32 h-9 justify-between active:scale-98 transition-all capitalize text-xs font-semibold"
                  >
              ...
              <DropdownMenuContent align="start" className="w-40">
                {["All Status", "Open", "Active", "Closed", "Lost"].map(
                  (item) => (
                    <DropdownMenuCheckboxItem
                      key={item}
                      ...
                      onCheckedChange={() => {
                        table
                          .getColumn("status")
                          ?.setFilterValue(
                            item === "All Status" ? undefined : item,
                          );
                      }}
                    >
                      {item}
                ...
                {["All Priority", "Low", "Medium", "High"].map((item) => (
                  <DropdownMenuCheckboxItem
                    ...
                    checked={
                      item === "All Priority"
                        ? !priorityFilter
                        : priorityFilter === item.toLowerCase()
                    }
                    onCheckedChange={() => {
                      table
                        .getColumn("priority")
                  ...
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => {
                      table
                        .getColumn("tags")
                        ?.setFilterValue(
                          tagsFilter === item ? undefined : item,
                        );
                    }}
                  >
                    {item}
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0 md:pt-4 lg:pt-0">
          <Button
            ...
            className="flex items-center gap-1.5 h-9 text-xs font-semibold active:scale-95 transition-all"
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-48 bg-card border border-border shadow-md rounded-md p-1"
            >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-card text-card-foreground border border-border shadow-2xl rounded-xl p-6 max-w-lg w-full mx-4 relative animate-in zoom-in-95 duration-200">
            ...
            <form
              onSubmit={editingLeadId ? handleEditLead : handleAddLead}
              className="space-y-4"
            >
